...
public class BaizelLangWriter1 {
    private final Writer output;

    public BaizelLangWriter1(OutputStream output) {
    ...
    enum NodeTypes {
        block, // made of statements
        statement, // made of values and optionally trailing block
        value // a string without spaces and '{};'
    }

    public void write(ObjectTree tree, int level) throws IOException {
        if (tree.isString() || tree.isPrimitive()) {
            output.write(tree.string());
            return;
        }
        var isBlockLevel = level % 2 == 0;
        if (tree.isList()) {
